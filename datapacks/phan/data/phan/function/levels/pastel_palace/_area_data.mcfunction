#pastel palace

#time attack times
scoreboard players set #timeBronze value 7000
scoreboard players set #timeSilver value 5000
scoreboard players set #timeGold value 4000
scoreboard players set #timeDiamond value 3000

#pearls, time limit, and score for each area

#act 1
scoreboard players set @s[tag=playing,scores={pCurrentArea=1}] pPearlsNeeded 16
scoreboard players set @s[tag=playing,scores={pCurrentArea=1}] pTimeRemaining 1419
scoreboard players set #scoreRankS1 value 70000
scoreboard players set #scoreRankA1 value 55000
scoreboard players set #scoreRankB1 value 30000
scoreboard players set #scoreRankC1 value 15000
scoreboard players set #targetTimeAct1 value 45

#act 2
scoreboard players set @s[tag=playing,scores={pCurrentArea=2}] pPearlsNeeded 16
scoreboard players set @s[tag=playing,scores={pCurrentArea=2}] pTimeRemaining 1819
scoreboard players set #scoreRankS2 value 80000
scoreboard players set #scoreRankA2 value 65000
scoreboard players set #scoreRankB2 value 35000
scoreboard players set #scoreRankC2 value 25000
scoreboard players set #targetTimeAct2 value 70

#act 3
scoreboard players set @s[tag=playing,scores={pCurrentArea=3}] pPearlsNeeded 12
scoreboard players set @s[tag=playing,scores={pCurrentArea=3}] pTimeRemaining 2009
scoreboard players set #scoreRankS3 value 70000
scoreboard players set #scoreRankA3 value 55000
scoreboard players set #scoreRankB3 value 30000
scoreboard players set #scoreRankC3 value 20000
scoreboard players set #targetTimeAct3 value 70