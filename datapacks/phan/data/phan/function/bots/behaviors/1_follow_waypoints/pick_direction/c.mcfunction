...
execute if score #waypointDir2 value matches 1.. in overworld run tag @e[type=armor_stand,tag=randomize,x=-1,y=-64,z=-1,dx=1,dy=1,dz=1,scores={r=2..4}] add directionViable
execute if score #waypointDir3 value matches 1.. in overworld run tag @e[type=armor_stand,tag=randomize,x=-1,y=-64,z=-1,dx=1,dy=1,dz=1,scores={r=5..17}] add directionViable
execute if score #waypointDir4 value matches 1.. in overworld run tag @e[type=armor_stand,tag=randomize,x=-1,y=-64,z=-1,dx=1,dy=1,dz=1,scores={r=18..20}] add directionViable

#ignore any directions we previously banned
execute if score #wpRestrictDirections value matches 1.. run function phan:bots/behaviors/1_follow_waypoints/pick_direction/c_restrict
#ignore any directions we don't want to backtrack to
execute unless score #wpRestrictDirections value matches 1.. if score #wpRestrictDirectionsBT value matches 1.. run function phan:bots/behaviors/1_follow_waypoints/pick_direction/c_restrict_backtrack
