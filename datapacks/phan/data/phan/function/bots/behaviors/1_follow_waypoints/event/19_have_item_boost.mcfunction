...
#deny if we don't have enough energy to boost
execute if score @s energy matches ..2 run return 0

#=====

#don't use items or boost for a moment
scoreboard players set @s[scores={botItemThinkTime=..29}] botItemThinkTime 30
scoreboard players set @s[scores={botBoostThinkTime=..59}] botItemThinkTime 60
