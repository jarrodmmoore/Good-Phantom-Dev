...
execute at @s facing entity @e[type=mannequin,tag=botSelf,distance=..11] eyes rotated ~ 0 run function phan:bots/reaction/mine_check_lane
...
execute if score #math value matches 1 run scoreboard players set @e[type=mannequin,tag=botSelf,distance=..1] botPreparedToDodgeTrap -20
execute if score #math value matches 2 run scoreboard players set @e[type=mannequin,tag=botSelf,distance=..1] botPreparedToDodgeTrap 20
