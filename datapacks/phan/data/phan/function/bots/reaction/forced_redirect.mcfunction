...
execute as @e[type=mannequin,tag=botSelf,distance=..1] if function phan:bots/reaction/forced_redirect_check_if_redundant run return 0

#keep checking really fast if we're kinda sorta in range
scoreboard players set @e[type=mannequin,tag=botSelf,distance=..1] botReactionTimer 2
...
execute as @e[type=mannequin,tag=botSelf,distance=..1] run function phan:bots/reaction/forced_redirect_go