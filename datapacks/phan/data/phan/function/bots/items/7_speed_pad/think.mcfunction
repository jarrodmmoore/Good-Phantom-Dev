...
#tryhard+ bot: hold onto at least 1 pad for waypoint shortcuts until final lap (unless we're the rival)
execute if score #vGameType value matches 1 if entity @s[tag=!botOnFinalLap,tag=!botFlightPanic,tag=!botRival,scores={botSkill=5..,botHasItem7=..1}] run return 0
