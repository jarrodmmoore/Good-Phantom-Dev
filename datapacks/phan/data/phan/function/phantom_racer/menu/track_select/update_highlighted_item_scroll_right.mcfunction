...
scoreboard players set @e[type=text_display,tag=setData,distance=..10] botID 6
tag @e[type=text_display,tag=setData,distance=..10] remove setData

#get ready to move everything over!
scoreboard players remove @e[type=text_display,tag=tvTrackDisplay,tag=!tvTracksBackground,distance=..10] botID 1