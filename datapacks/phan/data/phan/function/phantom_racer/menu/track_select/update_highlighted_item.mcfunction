...
execute as @e[type=text_display,tag=tvTrackDisplay,tag=!tvTracksBackground,distance=..10] run function phan:phantom_racer/menu/track_select/update_menu_item_position
...
execute as @e[type=text_display,tag=tvTrackDisplay,tag=!tvTracksBackground,distance=..10] if score @s boxID = #tvMenuState1 value run function phan:phantom_racer/menu/track_select/update_highlighted_item_check_model with storage phan:level_index
