...
$execute store result score @s vsRespawnX run data get storage phan_dream_$(level_id):pr_podium teleport_location[0]
$execute store result score @s vsRespawnY run data get storage phan_dream_$(level_id):pr_podium teleport_location[1]
$execute store result score @s vsRespawnZ run data get storage phan_dream_$(level_id):pr_podium teleport_location[2]
...
$execute store result score @s vsRespawnDimension run data get storage phan_dream_$(level_id):pr_podium teleport_dimension

#put in creative mode
gamemode creative @s

#let's also load conditions, too. why not?
$function phan:level_manager/warp_to_act_go_conditions with storage phan_dream_$(level_id):pr_podium
