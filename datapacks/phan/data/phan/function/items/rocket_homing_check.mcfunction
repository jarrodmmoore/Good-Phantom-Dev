...
execute if score #botCount value matches 1.. as @e[tag=ai,type=zombie] if score @s playerID = #checkID value run tag @s add rocketOwner
...
tag @a[tag=rocketOwner,distance=..25] remove rocketOwner