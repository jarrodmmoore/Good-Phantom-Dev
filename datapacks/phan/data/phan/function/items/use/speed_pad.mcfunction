clear @s[type=player] cyan_dye[custom_data~{speedPad:1b}] 1
...
#spawn speed pad
execute at @s run summon armor_stand ~ ~ ~ {Tags:["tickObject","speedPadEntity","giveID","hurtful","dontResetNearMe","speedPadCount"],attributes:[{id:"minecraft:scale",base:0.0625}],Invisible:1b,Invulnerable:1b,Passengers:[{id:"minecraft:item_display",brightness:{sky:0,block:15},Tags:["setLife","botObjectOfInterest","speedPadDisplay"],teleport_duration:1,item_display:"head",item:{id:"minecraft:cyan_dye",count:1b,components:{"item_model":"gp/item_entity/speed_pad"}}}]}
...
execute at @s run function phan:movement/speed_pad
