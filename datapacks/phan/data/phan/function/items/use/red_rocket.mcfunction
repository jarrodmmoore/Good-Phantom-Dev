clear @s[type=player] cyan_dye[custom_data~{redRocket:1b}] 1
...
summon armor_stand ~ ~1 ~ {Tags:["tickObject","rocket","giveID"],Marker:1b,Invisible:1b,Invulnerable:1b,Passengers:[{id:"minecraft:item_display",brightness:{sky:10,block:15},Tags:["setLife"],teleport_duration:1,item_display:"head",item:{id:"minecraft:cyan_dye",count:1b,components:{"item_model":"gp/item_entity/red_rocket"}}}]}
execute if entity @e[type=player] at @s positioned ~ ~1.6 ~ as @e[type=armor_stand,tag=giveID,distance=..4] run tp @s ~ ~ ~ ~ ~-1.5
execute if entity @e[type=!player] positioned ~ ~1.6 ~ as @e[type=armor_stand,tag=giveID,distance=..4] run tp @s ~ ~ ~ ~ ~-1.5
