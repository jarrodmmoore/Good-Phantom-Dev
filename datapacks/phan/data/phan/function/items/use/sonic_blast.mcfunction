...
execute if entity @s[type=player] as @e[type=armor_stand,tag=anvilEntity,distance=..9] if score @s playerID = #attackerID value run advancement grant @a[tag=blastSelf,distance=..1] only phan:portal_race/not_today_thank_you

#destroy some items
execute as @e[tag=sonicBlastDestroys,distance=..9] run function phan:items/use/sonic_blast_destroy_item

#redirect rockets
execute as @e[type=armor_stand,tag=rocket,distance=..12] run function phan:items/sonic_blast_redirect_rocket
...
execute if score #botsEnabled value matches 1.. as @e[type=mannequin,tag=ai,tag=!blastSelf,distance=..7] at @s run function phan:items/use/sonic_blast_hit_player
