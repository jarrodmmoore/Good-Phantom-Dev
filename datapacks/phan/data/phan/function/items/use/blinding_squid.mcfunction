...
execute if score #gameState value matches 4 if score #vGameType value matches 1 unless score @s itemPosition matches ..1 if score #botsEnabled value matches 1.. as @e[type=mannequin,tag=ai,tag=!dontgethit] if score @s itemPosition < #test value at @s run function phan:items/squid_blind_player
...
execute if score #botsEnabled value matches 1.. run scoreboard players set @e[type=mannequin,tag=ai,scores={botSkill=..5}] botHasItem10 0
execute if score #botsEnabled value matches 1.. run scoreboard players set @e[type=mannequin,tag=ai,scores={botHoldingItem=10}] botHoldingItem 0
