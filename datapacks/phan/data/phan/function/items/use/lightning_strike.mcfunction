...
execute if score #gameState value matches 4 if score #botsEnabled value matches 1.. as @e[type=mannequin,tag=ai,tag=!dontgethit] if score @s itemPosition matches ..5 if score @s itemLuck matches ..4 at @s run function phan:items/lightning_strike_hit_player
...
execute if score #botsEnabled value matches 1.. run scoreboard players set @e[type=mannequin,tag=ai,scores={botSkill=..5}] botHasItem9 0
execute if score #botsEnabled value matches 1.. run scoreboard players set @e[type=mannequin,tag=ai,scores={botHoldingItem=9}] botHoldingItem 0
