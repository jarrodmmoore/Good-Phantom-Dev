...
#play sound
playsound minecraft:entity.evoker.cast_spell master @s ~ ~ ~ 1 1 .75
...
execute if score #botsEnabled value matches 1.. run tag @e[type=mannequin,tag=anvilWillHit] remove anvilWillHit
...
execute if score #botsEnabled value matches 1.. run scoreboard players set @e[tag=ai,type=mannequin,scores={botSkill=..5}] botHasItem11 0
execute if score #botsEnabled value matches 1.. run scoreboard players set @e[tag=ai,type=mannequin,scores={botHoldingItem=11}] botHoldingItem 0
