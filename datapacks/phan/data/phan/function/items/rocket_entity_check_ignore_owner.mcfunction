...
execute if score #botsEnabled value matches 1.. as @e[tag=ai,type=mannequin,distance=..8] if score @s playerID = #checkID value run tag @s add rocketOwner
...
execute if score #botsEnabled value matches 1.. run tag @e[tag=rocketOwner,type=mannequin,distance=..8] remove rocketOwner
