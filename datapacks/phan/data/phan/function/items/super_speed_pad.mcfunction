...
execute if score #botsEnabled value matches 1.. unless score @s editArg1 matches 1 positioned ~-.5 ~-2 ~-.5 run damage @e[tag=ai,type=mannequin,dx=1,dy=1,dz=1,limit=1,scores={botMoveState=..2}] 4 mob_attack by @s from @s


#act like a speed pad (because we ARE one!)
execute positioned ~-1 ~-1 ~-1 as @a[gamemode=adventure,dx=2,dy=2,dz=2] at @s run function phan:movement/speed_pad_super
execute if score #botsEnabled value matches 1.. positioned ~-1 ~-1 ~-1 as @e[tag=ai,type=mannequin,dx=2,dy=2,dz=2] at @s run function phan:bots/movement/speed_pad_super
