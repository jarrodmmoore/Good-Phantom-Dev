...
scoreboard players set @e[tag=setMe,type=iron_golem,distance=..2] enemyMaxHP 550
...
#execute as @e[tag=setMe,type=iron_golem,distance=..2] store result score @s dEnemyHP run data get entity @s Health

#entity gets same rotation as whatever spawned it
execute as @e[tag=setMe,type=iron_golem,distance=..2] run tp @s ~ ~ ~ ~ ~

#join team so the golem doesn't attack other enemy mobs
team join colorRed @e[tag=setMe,type=iron_golem,distance=..2]

#clean up tag
tag @e[tag=setMe,type=iron_golem,distance=..2] remove setMe