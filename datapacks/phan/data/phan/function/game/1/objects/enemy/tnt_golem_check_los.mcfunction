...
execute facing entity @a[tag=playing,gamemode=adventure,distance=..42,limit=1,sort=nearest] eyes run function phan:game/1/objects/enemy/baby_ghast_check_los_recursive
...
execute if score #checkLOS value matches 1 run scoreboard players operation @s targetID = @a[tag=playing,gamemode=adventure,distance=..42,limit=1,sort=nearest] playerID
