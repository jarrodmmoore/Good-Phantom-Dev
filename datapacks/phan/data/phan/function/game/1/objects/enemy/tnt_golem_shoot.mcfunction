...
execute store result entity @e[limit=1,type=tnt,distance=..4,tag=giveVel] Motion[0] double 0.0156 run scoreboard players get #coord_dx value
execute store result entity @e[limit=1,type=tnt,distance=..4,tag=giveVel] Motion[1] double 0.0156 run scoreboard players get #coord_dy value
execute store result entity @e[limit=1,type=tnt,distance=..4,tag=giveVel] Motion[2] double 0.0156 run scoreboard players get #coord_dz value
tag @e[type=tnt,distance=..4,tag=giveVel,limit=1] remove giveVel

#get rid of that marker we used to grab second position
kill 00000309-0000-0000-0000-000000000001

#give color
team join colorRed @e[limit=1,type=tnt,distance=..4,tag=giveID]
#make sure it doesn't explode on ourselves
scoreboard players set @s playerID -1
scoreboard players set @e[limit=1,type=tnt,distance=..4,tag=giveID] playerID -1
tag @e[limit=1,type=tnt,distance=..4,tag=giveID] remove giveID
