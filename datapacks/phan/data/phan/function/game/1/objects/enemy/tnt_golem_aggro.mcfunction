...
execute as @a[tag=playing] if score @s playerID = #findTarget value run tag @s add tntGolemTarget

#face target
execute facing entity @a[limit=1,tag=tntGolemTarget] feet rotated ~ 0 run tp @s ~ ~ ~ ~ ~
...
#i also added the restriction that the golem must be holding a tnt (have the correct model) in order to initate an attack.
#this check is probably redundant, but you never know. i just want this to look good no matter what

#cleanup
tag @a[tag=tntGolemTarget] remove tntGolemTarget