...
execute if score #botsEnabled value matches 1.. positioned ~ ~0.4 ~ if entity @e[type=mannequin,tag=ai,distance=..2] if score #noExplode value matches 0 run scoreboard players set @s enemyHP 500
...
#slightly different animation for Trapped Chests
execute if entity @s[tag=trappedChest] run return run function phan:game/1/objects/enemy/mine_spin_trapped_chest
#=====
