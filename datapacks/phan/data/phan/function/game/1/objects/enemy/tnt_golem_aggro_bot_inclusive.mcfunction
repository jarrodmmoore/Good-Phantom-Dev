...
execute as @e[tag=playing,distance=..50] if score @s playerID = #findTarget value run tag @s add tntGolemTarget

#face target via /rotate
execute facing entity @e[limit=1,tag=tntGolemTarget,distance=..50] feet rotated ~ 0 run rotate @s ~ ~
...
execute if score @s editArg2 matches 25.. unless score @s editArg3 matches 1.. run function phan:game/1/objects/enemy/tnt_golem_throw_think_bot_inclusive
#i also added the restriction that the golem must be holding a tnt (have the correct model) in order to initiate an attack.
#this check is probably redundant, but you never know. i just want this to look good no matter what

#cleanup
tag @e[tag=tntGolemTarget,distance=..50] remove tntGolemTarget