...
execute if score #gameState value matches 4 if entity @s[tag=!ai,scores={hurtfulTime=1..,fallFlying=1..}] run tag @s add playerCancelMomentum
execute if score #gameState value matches 4 if entity @s[tag=ai,scores={hurtfulTime=1..,fallFlying=1..}] run function phan:bots/movement/2_gliding/lose_momentum
