...
execute if score #botsEnabled value matches 1.. as @e[tag=ai,type=mannequin] if score @s playerID = #projID value run tag @s add projectileOwner
...
execute if score #botsEnabled value matches 1.. run tag @e[tag=ai,type=mannequin,tag=projectileOwner] remove projectileOwner
