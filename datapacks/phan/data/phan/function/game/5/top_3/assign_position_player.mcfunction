...
data modify entity @e[type=area_effect_cloud,tag=setMe,distance=..1,limit=1] CustomName set from block 214 -16 130 front_text.messages[0]
tag @s remove self

#we are human (1 = human)
scoreboard players set @e[type=area_effect_cloud,tag=setMe,distance=..1] editArg2 1