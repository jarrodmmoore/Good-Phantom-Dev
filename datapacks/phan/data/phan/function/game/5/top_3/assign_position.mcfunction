...
#placement score
$scoreboard players set @e[type=area_effect_cloud,tag=setMe,distance=..1] editArg1 $(position)

#save points on aec as well
scoreboard players operation @e[type=area_effect_cloud,tag=setMe,distance=..1] versusPoints = @s versusPoints

#clean up tag
tag @e[type=area_effect_cloud,tag=setMe,distance=..1] remove setMe
