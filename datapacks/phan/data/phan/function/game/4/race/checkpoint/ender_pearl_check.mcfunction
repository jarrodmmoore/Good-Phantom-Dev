...
execute if score #botsEnabled value matches 1.. if score #coord_x value >= #checkMinX value if score #coord_x value <= #checkMaxX value if score #coord_y value >= #checkMinY value if score #coord_y value <= #checkMaxY value if score #coord_z value >= #checkMinZ value if score #coord_z value <= #checkMaxZ value as @e[type=mannequin,tag=ai] if score @s playerID = #getID value run function phan:game/4/race/checkpoint/player_hit_checkpoint_real