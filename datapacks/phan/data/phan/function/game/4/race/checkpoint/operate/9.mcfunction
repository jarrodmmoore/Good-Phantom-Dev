...
execute if score #check9relevant value matches 1 if entity @s[tag=posCalc] as @e[type=interaction,tag=playerPosMarker,scores={checkFake=8},sort=nearest] if score @s lap = #lapRecursion value run function phan:game/4/race/checkpoint/player_marker_get_position

#if we're an optional checkpoint, give check value to players for free regardless if they're in our trigger area
execute unless score #optionalCheckpoint value matches 0 as @a[tag=playing,scores={check=8}] run function phan:game/4/race/checkpoint/operate/checkpoint_add_1
...
#if we're an optional checkpoint, give check value to players for free regardless if they're in our trigger area
execute unless score #optionalCheckpoint value matches 0 as @e[tag=ai,type=mannequin,scores={check=8}] run function phan:game/4/race/checkpoint/operate/checkpoint_add_1
