...
execute if score #gameTime value matches 80..219 as @e[type=mannequin,tag=ai,tag=!eatMyShorts] at @s run function phan:game/4/race/start_grid/player
...
execute if score #gameTime value matches 81 if score #botsOnly value matches 1 as @a[tag=doneWithIntro,tag=!playing,gamemode=spectator] at @e[limit=1,sort=random,type=mannequin,tag=ai] positioned ^ ^2 ^-2 rotated ~ 45 run tp @s ~ ~ ~ ~ ~
