...
execute if score #eyeSpawnMethod value matches ..0 as @a[limit=1,sort=random,tag=playing,tag=!topHalfPlayers] at @s run tag @e[type=marker,tag=vsEyeSpawner,scores={versusSpawn=1,editArg3=1000..10000000},limit=3,sort=nearest] add mightSpawnHere
...
execute if score #eyeSpawnMethod value matches ..0 run tag @e[tag=mightSpawnHere] remove mightSpawnHere

#method 1 = spawn eye at random location NOT near one of the leading players
execute if score #eyeSpawnMethod value matches 1.. if score #test3 value >= #activeBattlePlayers value as @a[tag=playing,tag=topHalfPlayers] at @s run tag @e[type=marker,tag=vsEyeSpawner,tag=!dontSpawnHere,scores={versusSpawn=1,editArg3=1000..10000000},limit=1,sort=nearest] add dontSpawnHere
...
execute if score #eyeSpawnMethod value matches 1.. run tag @e[tag=dontSpawnHere] remove dontSpawnHere
