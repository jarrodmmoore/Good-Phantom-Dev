...
execute store result score #test value run execute if entity @e[type=armor_stand,tag=randomize,x=-1,y=-64,z=-1,dx=1,dy=1,dz=1,scores={r=1..17},tag=!nameTakenTryhard]
execute if score @s botSkill matches 5..6 if score #test value matches 1.. run scoreboard players operation #getID value = @e[limit=1,sort=random,type=armor_stand,tag=randomize,x=-1,y=-64,z=-1,dx=1,dy=1,dz=1,scores={r=1..17},tag=!nameTakenTryhard] r
