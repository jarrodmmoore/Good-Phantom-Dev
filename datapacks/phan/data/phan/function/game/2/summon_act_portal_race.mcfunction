...
#summon item
$execute positioned ~1 ~1 ~1 run summon item ~ ~ ~ {Tags:["stay","lobbyProp","actSelect"],PickupDelay:40,Age:-32768,NoGravity:1b,Item:{id:"minecraft:sea_lantern",count:1,components:{"minecraft:custom_name":{text:" ",color:"white",italic:false},"item_model":"gp/object/soul","minecraft:custom_data":{needsCleared:1b,actSelectItem:$(act)}}}}
...
#summon name text
$execute positioned ~-1 ~ ~1 run summon text_display ~ ~-10 ~ {Tags:["lobbyProp","rotateMe"],billboard:"fixed",alignment:"center",text:[{nbt:"act_name",storage:"phan_dream_$(level_id):pr_act_$(act)",interpret:true}]}
