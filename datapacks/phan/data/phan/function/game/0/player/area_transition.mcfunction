...
execute if score @s pGameTime matches ..100024 as @e[type=armor_stand,tag=currentCamera] at @s facing entity @e[limit=1,type=marker,tag=portalCore,sort=nearest] feet rotated ~ 0 if block ^ ^1 ^-.3 #phan:not_solid run tp @s ^ ^ ^-.03 ~ ~
...
execute if score #pGameTime value matches 100040.. as @e[type=armor_stand,tag=currentSoul] at @s run tp @s ~ ~1 ~
execute as @e[type=armor_stand,tag=currentSoul] at @s run function phan:game/1/player_soul_visuals_reduced
execute if score #pGameTime value matches 100000.. as @e[type=armor_stand,tag=currentSoul] at @s if entity @e[type=marker,tag=startPoint,distance=..5] run scoreboard players set #pGameTime value 100500
execute if score #pGameTime value matches 100040.. if score #10Hz value matches 0 as @e[type=armor_stand,tag=currentSoul] at @s run playsound minecraft:entity.allay.item_thrown master @a[tag=doneWithIntro,distance=..20] ~ ~ ~ 1 2 .5
...
execute if score #pGameTime value matches 100040..100140 if score #followStraightUp value matches 1 as @e[type=armor_stand,tag=currentCamera] at @s run function phan:game/1/area_transition_cam_follow_final
