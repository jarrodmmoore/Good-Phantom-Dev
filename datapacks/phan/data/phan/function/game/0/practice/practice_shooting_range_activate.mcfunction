...
execute if score #practiceTarget2 value matches 40.. as @e[type=marker,tag=node,scores={editNode=14},x=838,y=36,z=-1102,distance=..2] run function phan:game/1/spawning/target_setup
execute if score #practiceTarget2 value matches 40.. run scoreboard players set @e[type=marker,tag=node,scores={editNode=14}] editArg1 0
